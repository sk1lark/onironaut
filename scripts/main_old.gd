...
var spawn_interval: float = 1.5  # Balanced
var max_phantoms: int = 6  # Balanced
...
func start_game():
	...
	# Start with 3 phantoms for readability
	for i in range(3):
...
func _process(delta):
	if not can_accept_input and not choosing_upgrade:
		return
...
func trigger_typo():
	...
	# show miss indicator
	show_floating_text("miss", Vector2.ZERO, Color.RED)
...
func _on_power_up_collected(power_data: PowerUpData):
	...
	match power_data.power_type:
		"shield":
			...
			show_floating_text("shield", Vector2.ZERO, Color.CYAN)
		"slow":
			typing_speed_multiplier = max(0.5, typing_speed_multiplier - 0.2)
			await get_tree().create_timer(power_data.duration).timeout
			typing_speed_multiplier += 0.2
			show_floating_text("slow", Vector2.ZERO, Color.GREEN)
		"rapid":
			typing_speed_multiplier += 0.3
			await get_tree().create_timer(power_data.duration).timeout  
			typing_speed_multiplier -= 0.3
			show_floating_text("rapid", Vector2.ZERO, Color.YELLOW)
		"clear":
			...
			if active_phantoms.size() > 0:
				...
				closest.queue_free()
				show_floating_text("clear", Vector2.ZERO, Color.RED)
...
func _on_phantom_attacks(phantom: Phantom):
	...
	spawn_static_projectile(phantom.position, Vector2.ZERO)
	...
	spawn_static_burst(Vector2.ZERO)
